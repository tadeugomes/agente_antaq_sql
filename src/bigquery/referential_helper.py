...
class ReferentialHelper:
    ...
    def get_mercadoria_nome(self, cd_mercadoria: str) -> str:
        ...
        try:
            ...
            # Note: mercadoria_carga table has generic column names
            # string_field_0 = código, string_field_3 = descrição
            query = f"""
            SELECT string_field_3
            ...
            if not result.empty:
                nome = result.iloc[0]['string_field_3']
    ...
    def get_instalacao_destino_info(self, destino: str) -> Dict[str, str]:
        ...
        # 2. Check Brazilian ports table (uses 'origem' column, not 'destino')
        try:
            query = f"""
    ...
    def batch_get_mercadoria_nomes(self, codigos: list) -> Dict[str, str]:
        ...
        try:
            ...
            # string_field_0 = código, string_field_3 = descrição
            query = f"""
            SELECT string_field_0, string_field_3
            FROM `{self.dataset}.mercadoria_carga`
            ...
            for _, row in result.iterrows():
                code = str(row['string_field_0'])  # Ensure string type
                name = row['string_field_3']
